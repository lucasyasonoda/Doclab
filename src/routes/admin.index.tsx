...
import { BLOG_CATEGORIES } from "@/content/site";
import { contentFromEditor, contentToEditor } from "@/lib/blog-content";
...
function AdminDashboardContent() {
  ...
  function startEdit(article: (typeof articles)[0]) {
    ...
    setForm({
      ...
      content: contentToEditor(article.content),
    });
  }

  function parseContent(text: string) {
    return contentFromEditor(text);
  }
    ...
    <div className="animate-fade-in-up">
      <div className="mx-auto max-w-6xl px-6 py-8">
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-white/50">Crie, edite e publique os artigos do site.</p>
        </div>
          ...
          <div className="grid gap-8 lg:grid-cols-3">
            <section className="lg:col-span-1 rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-sm">
              ...
              <form onSubmit={isCreating ? handleCreate : handleUpdate} className="space-y-4">
                <div>
                  <label className="mb-1 block text-sm text-white/70">Link amigável (slug)</label>
                  <input
                    type="text"
                    required
                    value={form.slug}
                ...
                <div>
                  <label className="mb-1 block text-sm text-white/70">Resumo</label>
                  <textarea
                    required
                    rows={3}
                    value={form.excerpt}
                ...
                <div>
                  <label className="mb-1 block text-sm text-white/70">Conteúdo</label>
                  <p className="mb-1.5 text-xs text-white/40">
                    Escreva o artigo normalmente. Aceita HTML simples: &lt;h2&gt; para subtítulos,
                    &lt;p&gt; para parágrafos, &lt;ul&gt; e &lt;li&gt; para listas e
                    &lt;blockquote&gt; para destaques.
                  </p>
                  <textarea
                    rows={14}
                    value={form.content}
                    onChange={(e) => setForm({ ...form, content: e.target.value })}
                    className="w-full resize-y rounded-lg border-white/10 bg-white/5 px-3 py-2 font-mono text-xs text-gray-200 placeholder:text-white/30 focus:border-cyan focus:outline-none focus:ring-2 focus:ring-cyan/40"
                    placeholder={
                      "<p>Introdução do artigo...</p>\n<h2>Primeiro tópico</h2>\n<p>Desenvolvimento do tópico.</p>\n<ul>\n <li>Ponto importante</li>\n</ul>"
                    }
                  />
                </div>
